...
function styledFn(element, configOrCva = {}, options = {}) {
  const cvaFn = configOrCva.__cva__ || configOrCva.__recipe__
    ? configOrCva
    : cva(configOrCva);
  ...
  const StyledComponent = (props) => {
    ...
    const [forwardedProps, variantProps, bProps] = splitProps(
      aProps,
      forwardedKeys(),
    ...
    function recipeClass() {
      ...
      const compoundVariantStyles = __cvaFn__.__getCompoundVariantCss__?.(
        variantProps,
      );
      return cx(
